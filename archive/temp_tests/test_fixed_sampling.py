"""
Test the fixed sampling implementation.
"""
import os
import sys
import numpy as np
...
from frbayes_jax.sampling_fixed import run_nested_sampling_fixed
from frbayes_jax.utils import get_default_prior_ranges


def main():
    print("="*60)
    print("TEST: FIXED SAMPLING IMPLEMENTATION")
    print("="*60)
    ...
    # Run nested sampling with fixed implementation
    print("\nRunning nested sampling (fixed implementation)...")
    results = run_nested_sampling_fixed(
        ...
        num_delete=20,
        num_inner_steps=10,
        max_iterations=100,  # Just a quick test
        log_tolerance=-1.0,
        seed=123,
        verbose=True
    )
